...
function get_test_output( $fname, $fromtime )
{
    ...
    if ($cursection != '') {
        ...
        //error_log("admin/report/phpunittest/update.php: headers cursection = {$cursection}, tftime = {$lasttime}, percent = {$percentdone}  ");
        echo $outstr;
    }
...
//$len = strlen($sout);
//error_log("admin/report/phpunittest/update.php: filename={$filename} fromtime={$fromtime} - " . time() . " max time: " . ini_get('max_execution_time') . " ({$cnt}, $len) => {$sout}");
