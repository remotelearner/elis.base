 ...
 *
 * Code extracted from: lib/simpletestlib.php
 *
 * @copyright &copy; 2010 Remote-Learner
...
// Required for PHPUnit Test (TBD?)
//set_include_path("{$CFG->libdir}/pear" . PATH_SEPARATOR . get_include_path());

require_once('PHPUnit/Autoload.php');
//require_once('PHPUnit/Framework/Assert.php');
//require_once('PHPUnit/Framework/Test.php');
//require_once('PHPUnit/Framework/TestCase.php');
require_once($CFG->libdir.'/simpletestlib/expectation.php'); // TBD: use PHPUnit class

// Defines used by test_output() param#2 $divid
define('TEST_FILE_FORMAT', "<!-- test_output:name=\"%s\" time=%f status=%d -->%s\n");
define('TEST_OUTTER_SEARCH_ID',   'outter_search');
define('TEST_SEARCH_ID',          'search');
define('TEST_PROGRESS_ID',        'progress');
define('TEST_OUTTER_PROGRESS_ID', 'outter_progress');
define('TEST_RESULTS_ID',         'results');
define('TEST_COMPLETE_ID',        'complete');
 ...
 */
class prefix_changing_test_case extends PHPUnit_Framework_TestCase {
    var $old_prefix;
...
/**
 * test_output() - Method to replace echo, print, ...
 *                 redirecting testing output to file
 *
 * @param resource $outhandle - either file handle (or filename)
 * @param string $divid - the section the output is for (see defines above)
 * @param string $output - the output string
 * @param [optional] int $percentdone - percent complete - defaults to zero (0)
 *
 */
function test_output($outhandle, $divid, $output, $percentdone = 0)
{
    static $tnow;
    if (function_exists('microtime')) {
        $msec_sec = microtime(); // arg only good > PHP5 (must support PHP4)
        $tarray = explode(' ', $msec_sec);
        $tnow = $tarray[1] . substr($tarray[0], 1);
    } else {
        $stime = time();
        if ($stime == $tnow) {
            $tnow += 0.0001;
    ...
    // outhandle is (now) filename since update.php won't read from open file!
    if (($oh = fopen($outhandle, 'ab')) != false) {
        fwrite($oh, sprintf(TEST_FILE_FORMAT, $divid, $tnow, $percentdone, $output));
        //fflush($oh);
        fclose($oh);
    } // else TBD: error!
}
...
/**
 * FUNCTION MOVED TO: /admin/report/phpunittest/update.php
